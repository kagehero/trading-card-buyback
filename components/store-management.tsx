...
export function StoreManagement() {
  const { stores, loading, error, addStore, updateStore, deleteStore, moveStoreUp, moveStoreDown } = useStores()
  ...
  const handleEditStore = (store: Store) => {
    ...
    setOriginalImageUrl(store.imageUrl)

    if (store.imageUrls && store.imageUrls.length > 0) {
      setPreviewImageUrls(store.imageUrls)
      setPreviewImageFiles([])
      setSelectedImageIndex(0)
      setImageUrl(store.imageUrls[0])
    }
  ...
  const confirmDelete = async () => {
    if (storeToDelete) {
      ...
      try {
        const storeToDeleteObj = stores.find((store) => store.id === storeToDelete)
        await deleteStore(storeToDelete)

        if (storeToDeleteObj && storeToDeleteObj.imageUrl && !storeToDeleteObj.imageUrl.includes("/placeholder.svg")) {
          await deleteImage(storeToDeleteObj.imageUrl).catch((err) => {
            console.error("Failed to delete image, but store was deleted:", err)
          })
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.location || !formData.cardType || !formData.detailUrl) {
      alert("必須項目を入力してください。")
      return
    }
    ...
    try {
      ...
      if (previewImageFiles.length > 0) {
        ...
        try {
          ...
          const uploadPromises = previewImageFiles.map(async (file) => {
            return await uploadImage(file, "buyback-images")
          })
      ...
      if (isEditing && currentStoreId) {
        ...
        alert(`買取表が更新されました！（${finalImageUrls.length}枚の画像を保存）`)
      } else {
        await addStore(storeData)
        alert(`買取表が追加されました！（${finalImageUrls.length}枚の画像を保存）`)
      }
  ...
  const handleMoveUp = async (id: string) => {
    setOperationInProgress(true)
    try {
      await moveStoreUp(id)
    } catch (error) {
      console.error("Error moving store up:", error)
  ...
  const handleMoveDown = async (id: string) => {
    setOperationInProgress(true)
    try {
      await moveStoreDown(id)
    } catch (error) {
      console.error("Error moving store down:", error)
      alert("順序の変更に失敗しました。もう一度お試しください。")
    } finally {
      setOperationInProgress(false)
    }
  ...
  return (
    <div className="space-y-8">
      ...
      {stores.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold">既存のトレカ買取表</h2>
            ...
            {stores.map((store, index) => (
              <Card key={store.id} className="overflow-hidden">
                <div className="aspect-video overflow-hidden bg-muted">
                  <img
                    src={getResizedImageUrl(store.imageUrl, 400, 300) || "/placeholder.svg"}
                    alt={`${store.name}の買取表`}
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <Badge
                          className={`${getCardTypeColor(store.cardType).badgeBg} ${getCardTypeColor(store.cardType).badgeText} text-xs px-2 py-1 font-medium`}
                        >
                          {store.cardType}
                        </Badge>
                      </div>
                  ...
                  {showOrderControls ? (
                    <div className="flex gap-2 mt-4">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => handleMoveDown(store.id)}
                        disabled={index === stores.length - 1 || operationInProgress}
                      >
                        {operationInProgress ? (
                          <Loader2 className="h-4 w-4 animate-spin" />
                        ) : (
          ...
          {showOrderControls && (
            <div className="mt-4 p-4 bg-muted rounded-lg">
              <p className="text-sm text-muted-foreground">
                ...
                上下ボタンで買取表の表示順序を変更できます。変更は自動的に保存されます。
              </p>
            </div>
          )}
        </div>
      ...
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          <CardTitle>{isEditing ? "トレカ買取表を編集" : "新しいトレカ買取表を追加"}</CardTitle>
          <CardDescription>
            {isEditing
              ? "既存の買取表情報を編集してください。"
              : "店舗の買取表情報を入力してください。画像はドラッグ＆ドロップでもアップロードできます。"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="image-upload">買取表画像</Label>
              <div
                    ...
                    <div className="w-full space-y-4">
                      ...
                      {previewImageUrls.length > 1 && (
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-muted-foreground">選択された画像が買取表として使用されます</p>
                        </div>
                      )}
            ...
            <div className="space-y-2">
              ...
              <Input
                id="detail-url"
                placeholder="https://example.com/buyback-list"
